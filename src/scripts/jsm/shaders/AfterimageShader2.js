/**
 * Afterimage shader
 * I created this effect inspired by a demo on codepen:
 * https://codepen.io/brunoimbrizi/pen/MoRJaN?page=1&
 */

const AfterimageShader = {

	uniforms: {

		'damp': { value: 0.96 },
		'tOld': { value: null },
		'tNew': { value: null }

	},

	vertexShader: /* glsl */`

		varying vec2 vUv;

		void main() {

			vUv = uv;
			gl_Position = projectionMatrix * modelViewMatrix * vec4( position, 1.0 );

		}`,

	fragmentShader: /* glsl */`

		uniform float damp;

		uniform sampler2D tOld;
		uniform sampler2D tNew;

		varying vec2 vUv;

		vec4 when_gt( vec4 x, float y ) {

			return max( sign( x - y ), 0.0 );

		}

		void main() {
			float scl = 1.0111;
			float off = (1.0-scl)*.5;
			//vec4 texelOld = texture2D( tOld, vec2( off+vUv.x*scl, off+vUv.y*scl) );
			vec4 texelOld = texture2D( tOld, vec2( off+vUv.x*scl, off+vUv.y*scl) );
			vec4 texelNew = texture2D( tNew, vUv );

			//texelOld *= 0.101 / when_gt( texelOld, .2	 );
			texelOld *= .8 / when_gt( texelOld, 0.4	 );
			//gl_FragColor = (1.0-min(1.0-texelNew, (1.0 - vec4(texelOld.r,0.,0.,0.)) / .1025));
			gl_FragColor = (1.0-min(1.0-texelNew, (1.0 / texelOld.r) / .91025));
			//gl_FragColor = 1.-(1.0-min(1.0-texelNew, (1.0-texelOld) /0.1025));
			
			//texelOld *= 0.2 * when_gt( texelNew, .0192);
			//gl_FragColor = 1-max(texelNew, texelOld );
			
		}`

};

export { AfterimageShader };
